...
public static class FBGraph
{
	...
	{
		if (GameManager.Friends != null && GameManager.Friends.Count > 0) {
			GameManager.Friends.AddRange (newFriends);
		} else {
			GameManager.Friends = newFriends;
		}
		Debug.Log ("Saved friends");
	}
	...
	public static void SetScores(){
		...
		scoreData ["score"] = GameManager.HighScore.ToString();
		FB.API ("me/scores/", HttpMethod.POST, delegate(IGraphResult result) {
			Debug.Log ("score submit results: " + result.RawResult);
		}, scoreData);
	}
	...
	/*private static IEnumerator GetFriendsFromDatabase(List<object> dataList){
		Debug.Log ("Encoding friends to json array");
		string[] ids;
		foreach (object obj in dataList) {
			ids[obj] = dataList["id"];
			Debug.Log ("Id : " + ids [obj]);
		}

		Debug.Log ("Getting friends from database");
		var form = new WWWForm(); //here you create a new form connection
		var URL = "http://localhost/Data/GetAllSelfies.php"; //change for your URL
		...
		form.AddField( "fbIdsArray",friendsId);
		...
		if (w.error != null) {
			...
			Debug.Log(userFound);
			string theIdata = userFound.Split ("±");
			for (int i = 0; i < theIdata.Length; i++) {
				string theJdata = theIdata.Split ("|");
				for (int j = 0; j < theJdata.Length; j++) {
