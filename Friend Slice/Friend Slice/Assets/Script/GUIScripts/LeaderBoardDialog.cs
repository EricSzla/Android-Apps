...
public class LeaderBoardDialog : MonoBehaviour
{
	...
	void OnEnable ()
	{
		// When LeaderboardDialog is enabled, update the User Interface
		UpdateUI();
	}
	...
	public void UpdateUI ()
	{
		...
		if (GameStateManager.ScoresReady && scores.Count > 0)
		{
			...
			Transform[] childLBElements = LeaderboardContent.GetComponentsInChildren<Transform>();
			foreach(Transform childObject in childLBElements)
			{
				if(!LeaderboardContent.transform.IsChildOf(childObject.transform))
				{
			...
			for (int i=0; i<scores.Count; i++)
			{
				...
				LBelement.transform.SetParent (LeaderboardContent.transform, false);
			}
